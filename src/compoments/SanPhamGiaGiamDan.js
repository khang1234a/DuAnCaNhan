...
function SanPhamGiaGiamDan() {
    const [ListSPGiamDan, ganListSPGiamDan] = useState([]);
    useEffect(() => {
        fetch("http://localhost:3000/admin/giagiamdan")
            .then(res => res.json())
            .then(data => {
                if (data && data.data) {
                    ganListSPGiamDan(data.data);
                } else {
            ...
            <div className="container-product-admin">
                ...
                <div className="content-table-product-admin">
                    <table>
                        ...
                        <PhanTrang listSP={ListSPGiamDan} pageSize={12} />
                    </table>
                </div>
            </div>
        </div>
...
export default SanPhamGiaGiamDan;