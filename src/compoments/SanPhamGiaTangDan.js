...
function SanPhamGiaTangDan() {
    const [ListSPTangDan, ganListSPTangDan] = useState([]);
    useEffect(() => {
        fetch("http://localhost:3000/admin/giatangdan")
            .then(res => res.json())
            .then(data => {
                if (data && data.data) {
                    ganListSPTangDan(data.data);
                } else {
            ...
            <div className="container-product-admin">
                ...
                <div className="content-table-product-admin">
                    <table>
                        ...
                        <PhanTrang listSP={ListSPTangDan} pageSize={12} />
                    </table>
                </div>
            </div>
        </div>
...
export default SanPhamGiaTangDan;